...
class PerformanceLog:
    ...
    def tostr(self):
        strlist = ['--- data property ---',
                   'n_size:'+str(self.data_n)+','+'d_size:'+str(self.data_dim),
                   '--- total times ---',
                   str(self.timerDict),
                   '--- total counts ---',
...
class ChiSq(Module):
    ...
    def forward(self, losses, eta):
        ...
        """
        residual = losses - eta
        trunc_map = torch.sqrt(torch.sum(self.relu(residual) ** 2.0)) / (self.sqrtn)
        return trunc_map
...
class LipLossZO(Module):
    ...
    def __init__(self, radius, x_in, b_init):
        ...
        self.dists = Variable(torch.FloatTensor(dist_np), requires_grad=False).to(
            device
        )
        self.relu = torch.nn.ReLU()
...
class LipLoss(Module):
    ...
    def __init__(self, radius, x_in, b_init, k_dual=2):
...
class ConfLipLoss(Module):
    ...
    def forward(self, losses, eta):
        ...
        residual = losses.squeeze() - transport - eta
        trunc_map = torch.sqrt(torch.sum(self.relu(residual) ** 2.0)) / (self.sqrtn)
        penalty_term = torch.sum(bpos * self.dists * self.radius) / (self.sqrtn ** 2.0)
        confounding_penalty = (
            0.5 * self.delta * torch.sum(torch.abs(bpos)) / (self.sqrtn ** 2.0)
        )
...
class RKHSLoss(Module):
    ...
    def __init__(self, radius, x_in, b_init, kern_fn):
        ...
        self.kmat = Variable(torch.FloatTensor(kmat), requires_grad=False).to(device)
        self.relu = torch.nn.ReLU()
        self.b_var = Parameter(torch.FloatTensor(b_init))
    ...
    def forward(self, losses, eta):
        ...
        penalty_term = torch.sqrt(
            self.radius * torch.dot(self.b_var, torch.mv(self.kmat, self.b_var)) + 1e-20
        ) / (self.sqrtn ** 2.0)
        return trunc_map + penalty_term
...
class SimLinModel(Module):
    def __init__(self, input_dim, target_dim, use_bias=False, lamb=0):
        ...
        self.linear_half = torch.nn.Linear(self.half_dim, target_dim, bias=use_bias)
